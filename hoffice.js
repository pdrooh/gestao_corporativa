...
function addAnalista(date) {
    ...
    select.innerHTML = `
        ...
        ${getAnalistas().map(analista => `<option value="${analista.nome}" data-departamento="${analista.setor}" data-cargo="${analista.cargo}">${analista.nome}</option>`).join('')}
    `;
    ...
    updateSummary(); // Atualiza o resumo após adicionar um novo analista
}
...
function loadEscalas(year, month, day) {
    ...
    if (escalas[key] && escalas[key][day]) {
        escalas[key][day].forEach(analista => {
            ...
            const select = document.createElement('select');
            select.innerHTML = `
                <option value="${analista.nome}" data-departamento="${analista.departamento}" data-cargo="${analista.cargo}">${analista.nome}</option>
            `;
            container.appendChild(select);
        });
    }
